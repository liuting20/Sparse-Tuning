...
from models.helpers import complement_idx
...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        left_tokens = N - 1
        # print("===============223")
        # print(self.keep_rate)
        # print(keep_rate==1)
    
        # if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:
        if self.keep_rate < 1 or tokens is not None:  # double check the keep rate
            # print("===============225")
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=True):
        if keep_rate is None:
        ...
        if get_idx and index is not None:
            return x, n_tokens, idx,adapt_x
        return x, n_tokens, None,adapt_x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 global_pool=False,act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=True,tuning_config=None):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token,config =tuning_config)
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        # lt
        adapter_paraller_0 = x
        adapter_paraller_1 = []
        adapter_paraller_2 = []
        adapter_paraller_3 = []

        for i, blk in enumerate(self.blocks):
            x, left_token, idx,adapt_mlp = blk(x, keep_rate[i], tokens[i], get_idx)
            ...
            # lt
            if i < 3:
                adapter_paraller_0 = adapter_paraller_0+adapt_mlp
                
            if i ==2:
                x = x + adapter_paraller_0
                

            if i > 2 and i <6 :
                adapter_paraller_1.append(adapt_mlp)
               
            if i == 5:
                x = x + sum(adapter_paraller_1)
              

            if i > 5 and i <9 :
                adapter_paraller_2.append(adapt_mlp)
            if i == 8: 
                x = x + sum(adapter_paraller_2)
               

            if i > 8 and i <11 :
                adapter_paraller_3.append(adapt_mlp)
            if i == 11: 
                x = x + sum(adapter_paraller_3)
...
# lt
def vit_base_patch16(base_keep_rate=0.7, drop_loc=(3, 6, 9),**kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, keep_rate=keep_rate)
    model_kwargs.update(kwargs)
    model = _create_evit('deit_base_patch16_224', pretrained=False, **model_kwargs)
  
    # model = VisionTransformer(
    #     patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
    #     norm_layer=partial(nn.LayerNorm, eps=1e-6), base_keep_rate=0.7, drop_loc=(3, 6, 9),**kwargs)
    return model
